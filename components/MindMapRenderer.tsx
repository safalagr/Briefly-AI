...
const MindMapRenderer: React.FC<MindMapRendererProps> = ({ chart }) => {
    ...
    return (
        <div className="flex flex-col h-[600px] border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800/50 overflow-hidden shadow-sm relative">
            ...
            <div className="absolute top-4 right-4 z-10 flex flex-col gap-2">
                {/* Layout Controls */}
                <div className="bg-white dark:bg-slate-900 p-1 rounded-lg shadow-md border border-slate-200 dark:border-slate-700 flex flex-col gap-1">
                    <button 
                        onClick={() => setLayout('LR')} 
                        className={`p-2 rounded-md transition-colors ${layout === 'LR' ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 dark:bg-slate-800'}`}
                    ...
                    <button 
                        ...
                        className={`p-2 rounded-md transition-colors ${layout === 'TD' ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 dark:bg-slate-800'}`}
                ...
                {/* Zoom Controls */}
                <div className="bg-white dark:bg-slate-900 p-1 rounded-lg shadow-md border border-slate-200 dark:border-slate-700 flex flex-col gap-1">
                    <button onClick={() => setZoom(z => Math.min(4, z + 0.2))} className="p-2 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 dark:bg-slate-800 rounded-md" title="Zoom In">
                        <ZoomInIcon />
                    </button>
                    <button onClick={() => setZoom(z => Math.max(0.2, z - 0.2))} className="p-2 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 dark:bg-slate-800 rounded-md" title="Zoom Out">
                        <ZoomOutIcon />
                    </button>
                    <button onClick={resetView} className="p-2 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 dark:bg-slate-800 rounded-md" title="Reset View">
                        <RefreshIcon />
                    </button>
                </div>
            </div>
            ...
            {/* Viewport */}
            <div 
                className="flex-1 overflow-hidden relative bg-slate-50 dark:bg-slate-800/50 cursor-grab active:cursor-grabbing"
            ...
            >
                ...
                {renderError && (
                     <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                        <div className="bg-white dark:bg-slate-900/90 p-4 rounded-lg border border-red-200 text-red-600 shadow-lg text-sm font-medium">
            ...
            {/* Status Bar */}
            <div className="bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-700 px-4 py-2 text-xs text-slate-500 dark:text-slate-400 flex justify-between">
