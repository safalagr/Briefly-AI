...
const TranscriptView: React.FC<TranscriptViewProps> = ({ lecture, onBack, onUpdate, onDelete, onContinueRecording, isProcessing }) => {
    const isCombinedResource = lecture.tags?.includes('Combined Resources');
    const [activeTab, setActiveTab] = useState<DetailTab>(isCombinedResource ? 'summary' : 'transcript');
    ...
    useEffect(() => {
        ...
        setActiveTab(lecture.tags?.includes('Combined Resources') ? 'summary' : 'transcript');
    }, [lecture.id]);
    ...
    const handleGroupTopics = async () => {
        if (!lecture.chunks || lecture.chunks.length === 0) return;
        setIsLoading(true);
        setError(null);
        try {
            const groupedChunks = await GeminiService.groupTranscriptByTopic(lecture.chunks);
            const updatedLecture = { ...lecture, chunks: groupedChunks };
            StorageService.saveLecture(updatedLecture);
            onUpdate(updatedLecture);
        } catch (err: any) {
            setError(err.message || "Failed to group topics. Please try again.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const renderSummary = () => {
        ...
        return (
            <div className="space-y-6 animate-fadeIn pb-12">
                <div className="bg-blue-50 dark:bg-blue-900/30 p-6 rounded-xl border border-blue-100">
                ...
                {lecture.summary.mainPoints && lecture.summary.mainPoints.length > 0 && (
                    <div>
                        <h3 className="font-bold text-slate-800 dark:text-slate-100 text-lg mb-3">Main Points</h3>
                        <ul className="space-y-3">
                            {lecture.summary.mainPoints.map((point, i) => (
                                <li key={i} className="flex gap-3">
                                    <span className="flex-shrink-0 w-6 h-6 rounded-full bg-indigo-100 text-indigo-600 dark:text-indigo-400 flex items-center justify-center text-sm font-bold">{i + 1}</span>
                ...
                {lecture.summary.keyTerms && lecture.summary.keyTerms.length > 0 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {lecture.summary.keyTerms.map((term, i) => (
                            <div key={i} className="p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg shadow-sm">
                                <span className="block font-bold text-slate-900 dark:text-slate-50 mb-1">{term.term}</span>
                                <span className="text-sm text-slate-600 dark:text-slate-300">{term.definition}</span>
                            </div>
                        ))}
                    </div>
                )}
                ...
                <div className="flex justify-center mt-8">
                    <button
                        ...
                        disabled={isLoading}
                        className="flex items-center gap-2 px-6 py-2.5 bg-white dark:bg-slate-900 border border-indigo-200 text-indigo-600 dark:text-indigo-400 rounded-full font-medium hover:bg-indigo-50 dark:bg-indigo-900/30 shadow-sm transition-all disabled:opacity-70"
                    >
                        {isLoading ? "Expanding..." : (
                            <>
    ...
    const renderQuiz = () => {
        ...
        return (
            <div className="space-y-8 animate-fadeIn max-w-2xl mx-auto pb-8">
                ...
                {!isQuizSubmitted && (
                    <div className="flex justify-between items-center bg-indigo-50 dark:bg-indigo-900/30 p-5 rounded-2xl border border-indigo-100 shadow-sm">
                        <div>
                            <h3 className="font-bold text-indigo-900 text-lg">Knowledge Check</h3>
                            <p className="text-sm text-indigo-700 dark:text-indigo-300/80">Test your understanding of the lecture.</p>
                        </div>
                        <button
                            onClick={handleRevealAnswers}
                            className="flex items-center gap-2 text-sm font-semibold text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 bg-white dark:bg-slate-900 px-4 py-2 rounded-xl border border-indigo-100 shadow-sm hover:shadow-md transition-all active:scale-95"
                    ...
                    <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-6 text-center animate-fadeIn">
                        ...
                        <div className="flex flex-wrap justify-center gap-3 mt-6">
                            <button
                                ...
                                className="px-5 py-2 bg-white dark:bg-slate-900 border border-emerald-200 text-emerald-700 rounded-lg hover:bg-emerald-100 font-medium transition-colors"
                            >
                            ...
                            <button
                                ...
                                disabled={isLoading}
                                className="px-5 py-2 bg-white dark:bg-slate-900 border border-emerald-300 text-emerald-700 rounded-lg hover:bg-emerald-50 font-medium shadow-sm transition-colors flex items-center gap-2 disabled:opacity-70"
                            >
                                {isLoading ? "Adding..." : (
                                    <>
                ...
                {lecture.quiz.map((q, i) => {
                    ...
                    return (
                        <div key={i} className={`bg-white dark:bg-slate-900 p-6 rounded-xl border shadow-sm transition-all ${isQuizSubmitted
                                ? isCorrect
                            ...
                            <div className="flex gap-3 mb-4">
                                <span className="flex-shrink-0 w-8 h-8 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-full flex items-center justify-center font-bold text-sm">
                                    {i + 1}
                                </span>
                                <h4 className="font-bold text-lg text-slate-900 dark:text-slate-50 mt-0.5">{q.question}</h4>
                            </div>

                            <div className="space-y-2 ml-11">
                                {q.options.map((opt, idx) => {
                                    let optionClass = "border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 dark:bg-slate-800 text-slate-600 dark:text-slate-300";
                                    let icon = null;
                                    ...
                                    if (isQuizSubmitted) {
                                        if (isOptCorrect) {
                                            ...
                                            optionClass = "bg-red-50 dark:bg-red-900/30 border-red-300 text-red-800 font-medium ring-1 ring-red-300";
                                            icon = <XMarkIcon className="w-5 h-5 text-red-500" />;
                                        } else {
                                            optionClass = "border-slate-100 dark:border-slate-800 text-slate-400 dark:text-slate-500 opacity-60";
                                        }
                                    } else {
                                        if (normalizeText(userAnswer) === normalizeText(opt)) {
                                            optionClass = "bg-indigo-50 dark:bg-indigo-900/30 border-indigo-500 text-indigo-700 dark:text-indigo-300 ring-1 ring-indigo-500";
                                        }
                                    }
                            ...
                            {isQuizSubmitted && (
                                <div className="mt-4 ml-11 text-sm bg-slate-50 dark:bg-slate-800/50 p-3 rounded-lg border border-slate-100 dark:border-slate-800 text-slate-600 dark:text-slate-300 flex gap-2">
                ...
                {!isQuizSubmitted && (
                    <div className="flex flex-col items-center gap-4 pt-4">
                        ...
                        <button
                            onClick={handleRevealAnswers}
                            className="text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:text-indigo-400 text-sm font-medium px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-700 dark:bg-slate-800 rounded-lg transition-colors"
                        >
                            See All Answers
                        </button>
                    </div>
    ...
    const renderNotes = () => {
        ...
        return (
            <div className="relative">
                <div className="bg-white dark:bg-slate-900 p-8 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm animate-fadeIn mb-20">
                    <div
                ...
                <div className="fixed bottom-8 right-8 z-20 flex flex-col gap-2 items-end">
                    <button
                        ...
                        disabled={isLoading}
                        className="flex items-center gap-2 bg-indigo-100 text-indigo-700 dark:text-indigo-300 px-5 py-3 rounded-full font-semibold shadow-lg hover:bg-indigo-200 transition-all hover:scale-105 disabled:opacity-70 disabled:scale-100"
                    >
                        {isLoading ? (
                            <>
    ...
    const renderMindMap = () => {
        ...
        return (
            <div className="animate-fadeIn pb-12">
                <div className="mb-4">
                    <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">Visual Overview</h3>
                    <p className="text-sm text-slate-500 dark:text-slate-400">A hierarchical map of the key concepts in this lecture.</p>
                </div>
                ...
                <div className="flex justify-center mt-6">
                    <button
                        ...
                        className="px-4 py-2 text-sm text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:text-indigo-400 hover:bg-slate-100 dark:hover:bg-slate-700 dark:bg-slate-800 rounded-lg transition-colors flex items-center gap-2"
                    >
    ...
    const renderChat = () => {
        return (
            <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-800/50 relative animate-fadeIn">
                <div className="flex-1 overflow-y-auto p-4 space-y-4 mb-20">
                    {chatMessages.map((msg, i) => (
                        <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[80%] p-4 rounded-2xl text-sm leading-relaxed shadow-sm ${msg.role === 'user'
                                    ...
                                    : 'bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 rounded-bl-none prose prose-sm dark:prose-invert'
                                }`}>
                                {msg.role === 'model' ? (
                    ...
                    {isChatLoading && (
                        <div className="flex justify-start">
                            <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl rounded-bl-none border border-slate-200 dark:border-slate-700">
                ...
                <div className="absolute bottom-4 left-4 right-4">
                    <div className="flex gap-2 p-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-full shadow-lg">
                        <input
                            ref={chatInputRef}
                            type="text"
                            value={chatInput}
                            ...
                            className="flex-1 px-4 py-2 bg-transparent focus:outline-none text-slate-700 dark:text-slate-200 placeholder:text-slate-400 dark:text-slate-500"
                            autoFocus
                        />
                        <button
    ...
    return (
        <div className="flex flex-col h-full bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
            {/* Header */}
            <div className="bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 p-4 flex items-center justify-between sticky top-0 z-10">
                <div className="flex items-center gap-4 flex-1 overflow-hidden">
                    <button onClick={onBack} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 dark:bg-slate-800 rounded-full transition-colors text-slate-500 dark:text-slate-400 flex-shrink-0">
                        ...
                        {isEditingTitle ? (
                            <div className="flex items-center gap-2">
                                <input
                                    ...
                                    autoFocus
                                    className="font-bold text-xl text-slate-900 dark:text-slate-50 bg-white dark:bg-slate-900 border border-indigo-300 rounded px-2 py-1 w-full focus:outline-none focus:ring-2 focus:ring-indigo-200"
                                />
                                <button onClick={handleTitleSave} className="p-1 text-green-600 hover:bg-green-50 dark:bg-green-900/30 rounded">
                            ...
                            <h2
                                ...
                                className="font-bold text-xl text-slate-900 dark:text-slate-50 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700 dark:bg-slate-800 hover:text-indigo-700 dark:text-indigo-300 transition-colors rounded px-1 -ml-1 border border-transparent hover:border-slate-200 dark:border-slate-700 truncate"
                        ...
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-2 mt-1">
                            ...
                            <div className="flex flex-wrap gap-2 items-center">
                                {lecture.tags && lecture.tags.map(tag => (
                                    <span key={tag} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-xs font-medium border border-slate-200 dark:border-slate-700 group">
                                        <HashtagIcon className="w-3 h-3 text-slate-400 dark:text-slate-500" />
                                        {tag}
                                        <button onClick={() => handleRemoveTag(tag)} className="text-slate-400 dark:text-slate-500 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                {isAddingTag ? (
                                    ...
                                    <button
                                        ...
                                        className="text-xs text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 hover:bg-indigo-50 dark:bg-indigo-900/30 px-2 py-0.5 rounded transition-colors font-medium"
                                    >
                                        + Add Tag
                                    </button>
                                )}
                ...
                <div className="flex gap-2 ml-4 flex-shrink-0">
                    {isProcessing && (
                        <div className="flex items-center gap-2 px-3 py-1 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 rounded-full text-xs font-semibold animate-pulse border border-indigo-100 whitespace-nowrap">
                    ...
                    <button
                        ...
                        className="text-red-400 hover:text-red-600 p-2 rounded-full hover:bg-red-50 dark:bg-red-900/30 transition-colors cursor-pointer relative z-50"
            ...
            {/* Navigation Tabs */}
            <div className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700 px-4 flex gap-1 overflow-x-auto">
                {!isCombinedResource && (
            ...
            {/* Content Area */}
            <div className="flex-1 overflow-y-auto p-0 bg-slate-50 dark:bg-slate-800/50 relative">
                {error && activeTab !== 'chat' && (
                    <div className="m-6 p-4 bg-red-50 dark:bg-red-900/30 text-red-700 border border-red-200 rounded-lg">
                        {error}
                    </div>
                )}
                        ...
                        {activeTab === 'transcript' && (
                            <div className="space-y-4">
                                <div className="flex justify-end gap-2">
                                    {lecture.chunks && lecture.chunks.length > 0 && !lecture.chunks[0].topic && (
                                        <button
                                            onClick={handleGroupTopics}
                                            disabled={isLoading}
                                            className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-900 border border-indigo-200 text-indigo-600 dark:text-indigo-400 text-sm font-medium rounded-lg hover:bg-indigo-50 dark:hover:bg-slate-800 shadow-sm transition-colors disabled:opacity-70"
                                        >
                                            <SparklesIcon className="w-4 h-4" />
                                            {isLoading ? "Grouping..." : "Group by Topics"}
                                        </button>
                                    )}
                                    <button
                                ...
                                {lecture.chunks && lecture.chunks.length > 0 ? (
                                    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-6 space-y-1">
                                        {lecture.chunks.map((chunk, idx) => (
                                            <div key={idx} className={`flex gap-4 group hover:bg-slate-50 dark:hover:bg-slate-700 dark:bg-slate-800 p-2 -mx-2 rounded-lg transition-colors items-baseline ${chunk.topic ? 'mt-4 border border-slate-100 dark:border-slate-800' : ''}`}>
                                                <div className="w-20 flex-shrink-0 text-right font-mono text-xs text-slate-400 dark:text-slate-500 group-hover:text-indigo-500 select-none pt-1">
                                                    {formatTime(chunk.timestamp)}
                                                </div>
                                                <div className="flex-1 text-slate-700 dark:text-slate-200 leading-relaxed">
                                                    {chunk.topic && <div className="font-bold text-indigo-700 dark:text-indigo-400 text-lg mb-2">{chunk.topic}</div>}
                                                    {chunk.text}
                                                </div>
                                            </div>
                                        ))}
                                    ...
                                    lecture.sessions.map((session, idx) => (
                                        <div key={session.id} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                                            ...
                                            <div className="p-8">
                                                <p className="whitespace-pre-wrap leading-relaxed text-slate-700 dark:text-slate-200">{session.text}</p>
                                            </div>
                                        </div>
                                    ))
                                ) : (
                                    <div className="bg-white dark:bg-slate-900 p-8 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                                        <p className="whitespace-pre-wrap leading-relaxed text-slate-700 dark:text-slate-200">{lecture.transcriptText}</p>
                                    </div>
                                )}
                            </div>
                        )}
...
const TabButton = ({ children, active, onClick, icon }: any) => (
    <button
        ...
        className={`flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 transition-colors ${active
                ...
                : 'border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 dark:bg-slate-800'
            }`}
    >
        {icon}
        {children}
...
const EmptyState = ({ icon, title, description, action, loading }: any) => (
    <div className="flex flex-col items-center justify-center h-64 md:h-96 text-center">
        <div className="mb-4 p-4 bg-white dark:bg-slate-900 rounded-full shadow-sm border border-slate-100 dark:border-slate-800">{icon}</div>
    ...
    return (
        ...
        >
            <div className={`relative w-full h-full transition-all duration-500 transform-style-3d ${flipped ? 'rotate-y-180' : ''}`}>
                {/* Front (Question) */}
                <div className="absolute inset-0 w-full h-full bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col backface-hidden">
                    ...
                    <div className="flex-1 flex items-center justify-center">
                        <p className="text-xl font-medium text-slate-800 dark:text-slate-100 text-center leading-relaxed">{card.front}</p>
                    </div>
                    <div className="mt-4 text-center">
                        <span className="text-xs text-slate-400 dark:text-slate-500 font-medium bg-slate-50 dark:bg-slate-800/50 px-3 py-1 rounded-full">Click to Reveal Answer</span>
                    </div>
                </div>
                ...
                <div className="absolute inset-0 w-full h-full bg-slate-900 p-6 rounded-xl shadow-lg flex flex-col backface-hidden rotate-y-180">
                    <div className="flex justify-between items-start mb-4">
                        <span className="text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Answer</span>
                    </div>
                    ...
                    <div className="mt-4 text-center flex justify-between items-center text-xs text-slate-500 dark:text-slate-400 border-t border-slate-800 pt-3">
                        <span>{card.topic}</span>
                        <span className="text-slate-400 dark:text-slate-500">Click to flip back</span>
                    </div>
                </div>
            </div>
        </div>
