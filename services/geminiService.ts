...
import { Flashcard, QuizQuestion, SummaryData, ChatMessage, TranscriptChunk } from '../types';
...
export const groupTranscriptByTopic = async (chunks: TranscriptChunk[]): Promise<TranscriptChunk[]> => {
    const ai = getAiClient();
    
    // We pass the chunks as a structured JSON so the LLM has exact text and timestamps
    const chunksData = chunks.map(c => ({ t: Math.floor(c.timestamp), text: c.text }));
    const stringified = JSON.stringify(chunksData);
    
    const response = await ai.models.generateContent({
        model: MODEL_FAST,
        contents: `I have a transcript consisting of multiple small chunks with timestamps (t in seconds).
        Group these chunks into logical topics. For each topic:
        1. Identify the starting timestamp (t) of the topic EXACTLY as it appears in the chunks.
        2. Provide a short, descriptive topic title.
        
        RAW CHUNKS DATA:
        ${stringified.slice(0, 3000000)}`,
        config: {
            ...
            responseSchema: {
                ...
                items: {
                    type: Type.OBJECT,
                    properties: {
                        timestamp: { type: Type.NUMBER, description: "The starting timestamp of the topic in seconds (must exactly match a 't' from the input)" },
    ...
    if (response.text) {
        const topicBoundaries = JSON.parse(response.text) as { timestamp: number; topic: string }[];
        
        // Ensure boundaries are sorted by timestamp
        topicBoundaries.sort((a, b) => a.timestamp - b.timestamp);
        
        const groupedChunks: TranscriptChunk[] = [];
        let currentGroup: TranscriptChunk | null = null;
        let boundaryIndex = 0;

        for (const chunk of chunks) {
            let triggeredNewTopic = false;
            let latestTopic = "";
            
            // Advance boundaryIndex if we've reached or passed the next topic boundary
            while (boundaryIndex < topicBoundaries.length && Math.floor(chunk.timestamp) >= topicBoundaries[boundaryIndex].timestamp) {
                triggeredNewTopic = true;
                latestTopic = topicBoundaries[boundaryIndex].topic;
                boundaryIndex++;
            }
            
            if (triggeredNewTopic) {
                if (currentGroup) {
    ...
    throw new Error("Failed to group transcript by topic");
};
...
export const generateSummary = async (transcript: string): Promise<SummaryData> => {
    ...
    const response = await ai.models.generateContent({
        ...
        TRANSCRIPT:
        ${transcript.slice(0, 3000000)}`, // Limit context window safely
        config: {
            responseMimeType: "application/json",
            responseSchema: {
                type: Type.OBJECT,
...
export const createLectureChat = (transcript: string, history: ChatMessage[] = []): Chat => {
    ...
    const safeTranscript = transcript ? transcript.slice(0, 3000000) : "No transcript available.";
        ...
        contents: [
            {
                ...
                parts: [
                    imagePart,
                    { text: "Please extract all readable text from this image exactly as it appears. If there are any diagrams, charts, or important visual structures, describe them in detail so the information is not lost. The output should be comprehensive and suitable for studying." }
                ]
            }
        ]
    });
